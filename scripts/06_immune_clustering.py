...
print("\n=== Seçici entegrasyon UMAP'i: bağışıklık karışık, diğerleri doku-özgü ===")
sc.pl.embedding(
...
print("\n=== Tek Global UMAP Çiziliyor (Ayrı dosya/grafik yok) ===")
sc.pl.umap(
    adata,
    color=["celltype", "dataset"],  
    frameon=False,
    title="Tüm Hücreler Tek Grafikte (Sadece İmmünler Entegre)"
)

adata_immune = adata[adata.obs["celltype"].isin(immune_classes)].copy()

print(f"\nToplam hücre: {adata.shape[0]}")
print(f"Bağışıklık hücresi: {adata_immune.shape[0]}")
print(f"Oran: {adata_immune.shape[0] / adata.shape[0] * 100:.1f}%")

print("\nBağışıklık hücresi tipi dağılımı:")
print(adata_immune.obs["celltype"].value_counts())

print("\nDoku bazında bağışıklık hücresi dağılımı:")
