...
for ct in target_cell_types:
    print(f"\n==========================================")
    print(f"İşleniyor & Alt Tiplendiriliyor: {ct}")
    print(f"==========================================")
    
    sub_adata = adata_immune[adata_immune.obs["immune_celltype"] == ct].copy()
    if sub_adata.shape[0] < 30: 
        print(f"Yetersiz hücre ({sub_adata.shape[0]}), atlanıyor.")
        continue
    ...
    if score_cols:
        ...
        sc.pl.umap(
            ...
            size=35,
            show=True,
            title=[f"{ct} Leiden", f"{ct} Otomatik Alt Tipler", f"{ct} Alt Tipler (Dağılım)"]
        )
    else:
        print(f"{ct} için geçerli marker gen eşleşmesi bulunamadı.")
